...
";
    ...
    function test_createRequestCountsWith()
    {
        ...
        $this->assertEquals('executeUpdate', $updateCall[0], 'call 1');
        $this->assertEquals(
    ...
    function test_countWhereLoginsSucceededSpecifiedAfter()
    {
        ...
        $params = $this->requestData1;
        unset($params['dtFrom']);
        $params['dtLimit'] = $this->dtLimit;
        $this->assertEquals($params, $qb->getParameters(), 'parameters');
    ...
    function test_incrementCount()
    {
        ...
        $qb = $this->wrapper->calls[0][2];
        $this->assertEquals("SELECT r.id FROM secu_requests r WHERE (r.username = :username) AND (r.ipAddress = :ipAddress) AND (r.dtFrom = :dtFrom) AND (r.cookieToken = :token) AND (addressReleasedAt IS NULL) AND (userReleasedAt IS NULL) AND (userReleasedForAddressAndCookieAt IS NULL)"
            , $qb->getSQL());
        $this->assertEquals($this->requestData1, $qb->getParameters(), 'parameters');
        $id = self::$connection->fetchColumn($qb->getSQL(), $qb->getParameters());
...
//        $result = self::$connection->fetchColumn($qb->getSQL(), $qb->getParameters());
    ...
    function test_updateCountsColumnWhereColumnNullAfterSupplied()
    {
        ...
        $qb = $this->wrapper->calls[0][2];
        $this->assertEquals(
               "UPDATE secu_requests SET userReleasedForAddressAndCookieAt = :value WHERE (userReleasedForAddressAndCookieAt IS NULL) AND (dtFrom > :dtLimit) AND (username = :username) AND (ipAddress = :ipAddress) AND (cookieToken = :token)"
               , $qb->getSQL()
           );
        $params = $this->requestData1;
        $params['value'] = $this->requestData1['dtFrom'];
        unset($params['dtFrom']);
        $params['dtLimit'] = $this->dtLimit;
        $this->assertEquals($params, $qb->getParameters(), 'parameters');
        ...
        $this->assertNull($result[0]['userReleasedAt'], 'userReleasedAt');
        $this->assertNull($result[0]['addressReleasedAt'], 'addressReleasedAt');
        $this->assertEquals($params['value'], $result[0]['userReleasedForAddressAndCookieAt'], 'userReleasedForAddressAndCookieAt');
    }
    ...
    function test_deleteCountsUntil()
    {
        ...
        $params = ['dtLimit' => $now->format('Y-m-d H:i:s')];
        $this->assertEquals($params, $qb->getParameters(), 'parameters');
    ...
    function test_countsGroupedByIpAddress()
    {
        ...
        $this->assertEquals('fetchAll', $this->wrapper->calls[0][0], 'connection method called');
    ...
    function test_insertRelease()
    {
        ...
        $this->assertEquals('fetchColumn', $this->wrapper->calls[0][0], 'connection method called');
        $callParams = $this->wrapper->calls[0][1];
        $this->assertEquals(
        ...
        $this->assertEquals($expectedParams, $callParams[1], 'parameters');
        $id = $this->wrapper->calls[0][2];
        $this->assertFalse($id, 'id found');

        $this->assertEquals('executeUpdate', $this->wrapper->calls[1][0], 'connection method called');
    ...
    function test_updateRelease()
    {
        ...
        $this->assertEquals('fetchColumn', $this->wrapper->calls[0][0], 'first connection method called');
        $callParams = $this->wrapper->calls[0][1];
        $this->assertEquals(
            "SELECT id from secu_releases WHERE username = :username AND ipAddress = :ipAddress AND cookieToken = :cookieToken"
            , $callParams[0]
        ...
        $this->assertEquals('executeUpdate', $this->wrapper->calls[1][0], 'second connection method called');
        $callParams = $this->wrapper->calls[1][1];
        $this->assertEquals(
    ...
    function test_deleteReleasesUntil()
    {
        ...
        $this->assertEquals("DELETE FROM secu_releases WHERE releasedAt < :dtLimit"
            , $qb->getSQL());
        $this->assertEquals(['dtLimit' => $this->dtLimit], $qb->getParameters(), 'parameters');
